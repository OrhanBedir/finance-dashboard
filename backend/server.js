...
// ── GEÇİCİ TEŞHİS UÇLARI (izolasyon hata ayıklama — sonra silinecek) ─────────
// 1) Bir tenant şemasının provizyon durumunu doğrudan kontrol eder (raw client).
app.get("/debug/tenant-check", async (req, res) => {
  const slug = String(req.query.slug || "").toLowerCase().replace(/[^a-z0-9_]/g, "");
  if (!slug) return res.json({ ok: false, error: "slug gerekli" });
  ...
  try {
    ...
    const tbls = await client.query(`SELECT count(*)::int AS n FROM pg_tables WHERE schemaname=$1`, [schema]);
    let ie = null, ieErr = null, ieSchema = null;
    try {
      const c = await client.query("SELECT count(*)::int AS n FROM invoice_entries");
      ie = c.rows[0].n;
      const w = await client.query("SELECT (SELECT nspname FROM pg_namespace n JOIN pg_class c2 ON c2.relnamespace=n.oid WHERE c2.relname='invoice_entries' AND n.nspname=ANY(current_schemas(false)) LIMIT 1) AS sch");
      ieSchema = w.rows[0].sch;
    } catch (e) { ieErr = e.message; }
    res.json({ ok: true, slug, schema, isolated_in_allowlist: isIsolatedTenant(slug), search_path: sp.rows[0].sp, tables_in_schema: tbls.rows[0].n, invoice_entries_count: ie, invoice_entries_resolved_schema: ieSchema, ie_error: ieErr });
  } catch (e) {
...
// 2) Bir token'ın gerçekte hangi şemadan okuduğunu gösterir (patched pool.query + ALS yolu).
app.get("/debug/whoami", authMiddleware, async (req, res) => {
  const tenant = req.user?.tenant || "erc";
  let search_path = null, ie = null, err = null;
  try {
    const sp = await pool.query("SELECT current_setting('search_path') AS sp");
    search_path = sp.rows[0]?.sp;
    const c = await pool.query("SELECT count(*)::int AS n FROM invoice_entries");
    ie = c.rows[0]?.n;
